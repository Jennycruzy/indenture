...
/// @title DeployIndenture — deploys the shared INDENTURE backbone to Sepolia.
...
///         addresses by chainId. Run via `scripts/deploy-indenture-sepolia.sh`.
contract DeployIndenture is Script {
    function run() external {
        ...
        Indenture engine = new Indenture();
        DemoConfidentialToken token = new DemoConfidentialToken("Indenture USD", "iUSD", "");
        ConfidentialFeed feed = new ConfidentialFeed(publisher);
        vm.stopBroadcast();

        console.log("== INDENTURE backbone deployed ==");
        console.log("Indenture (engine):    %s", address(engine));
